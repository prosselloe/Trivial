...
class CiberPuntCompresScreen extends StatefulWidget {
  final bool isCheeseSquare;
  const CiberPuntCompresScreen({super.key, this.isCheeseSquare = false});

  @override
  State<CiberPuntCompresScreen> createState() => _CiberPuntCompresScreenState();
}

class _CiberPuntCompresScreenState extends State<CiberPuntCompresScreen> {
  ...
  Future<void> _initialize() async {
    ...
    setState(() {
      ...
      // CORREGIT: Crida a getRandomQuestion amb un sol argument
      _currentQuestion = quizService.getRandomQuestion('CompresEnLinia');
    });
  }
  ...
  Future<void> _playSound(bool isCorrect) async {
    ...
    try {
      ...
      // Gestiona errors de reproducció si cal
    }
  }
  ...
  void _handleAnswer(String selectedOption) {
    ...
    // Actualitza la puntuació (correctes/incorrectes)
    quizService.actualitzarPuntuacio(_activePlayer!, category, isCorrect);

    // Si la resposta és correcta i és una casella de formatget, atorga el formatget
  ...
  void _moveToNextTurn(bool isCorrect) async {
    // Si la resposta no és correcta, passa al següent jugador
    if (!isCorrect) {
      final prefs = await SharedPreferences.getInstance();
    ...
    // Espera 3 segons abans de tornar a la pantalla del dau
  ...
  Widget build(BuildContext context) {
    if (_currentQuestion == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Punt: Compres Segures')),
        body: Center(
          child: Column(
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Image.asset('assets/images/osi-trivial-ciberpunt-compres-en-linia.jpg'),
            const SizedBox(height: 20),
            Text(
