...
class CiberPuntNavegacioScreen extends StatefulWidget {
  final bool isCheeseSquare;
  const CiberPuntNavegacioScreen({super.key, this.isCheeseSquare = false});

  @override
  State<CiberPuntNavegacioScreen> createState() => _CiberPuntNavegacioScreenState();
}

class _CiberPuntNavegacioScreenState extends State<CiberPuntNavegacioScreen> {
  ...
  Future<void> _initialize() async {
    ...
    setState(() {
      ...
      _currentQuestion = quizService.getRandomQuestion('NavegacioSegura');
    });
  }
  ...
  void _handleAnswer(String selectedOption) {
    ...
    final quizService = Provider.of<QuizService>(context, listen: false);
    const category = 'NavegacioSegura';
 ...
 @override
  Widget build(BuildContext context) {
    if (_currentQuestion == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Punt: Navegació Segura')),
        body: Center(
          child: Column(
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Image.asset('assets/images/osi-trivial-ciberpunt-navegacio-segura.jpg'),
            const SizedBox(height: 20),
            Text(
