...
class CiberPuntDispositiusScreen extends StatefulWidget {
  final bool isCheeseSquare;
  const CiberPuntDispositiusScreen({super.key, this.isCheeseSquare = false});

  @override
  State<CiberPuntDispositiusScreen> createState() => _CiberPuntDispositiusScreenState();
}

class _CiberPuntDispositiusScreenState extends State<CiberPuntDispositiusScreen> {
  ...
  Future<void> _initialize() async {
    ...
    setState(() {
      ...
      _currentQuestion = quizService.getRandomQuestion('ProteccioDispositius');
    });
  }
  ...
  void _handleAnswer(String selectedOption) {
    ...
    final quizService = Provider.of<QuizService>(context, listen: false);
    const category = 'ProteccioDispositius';
 ...
 @override
  Widget build(BuildContext context) {
    if (_currentQuestion == null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Punt: Protecció de Dispositius')),
        body: Center(
          child: Column(
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            Image.asset('assets/images/osi-trivial-ciberpunt-proteccio-de-dispositius.jpg'),
            const SizedBox(height: 20),
            Text(
